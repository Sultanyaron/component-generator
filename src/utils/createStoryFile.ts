...
export const createStoryFile = async (destinationPath: string, componentName: string) => {
  const storyTemplate = await readStoryAndApplyComponentName(componentName);

  await saveTemplateToDestination(destinationPath, `${componentName}.stories.tsx`, storyTemplate);
};
